...
const PlusCounter = memo(
  ({ count, onClick, gradientFrom = "#F9D769", gradientTo = "#734D20" }) => {
...
const TopInfluencers = () => {
      ...
      return {
        src:
          ...
          `https://ui-avatars.com/api/?name=${encodeURIComponent(
            ...
            user.last_name
          )}&background=F9D769&color=734D20&size=128&rounded=true`,
  ...
  const fetchTopInfluencers = useCallback(
    async (showToast = false) => {
      try {
        ...
        if (response?.status === 200 && response.data?.actioned_users_top) {
          const transformedData = transformInfluencersData(
            response.data.actioned_users_top
          );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Most active users in your campaigns
          </p>
        </div>
      </div>
